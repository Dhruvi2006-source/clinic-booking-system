...
export default function DoctorQuickFilters({ quickFilter, onToggle }: QuickFiltersProps) {
  return (
    <div className="flex flex-wrap gap-3 mt-2 items-center justify-center">
      <span className="text-gray-900 font-caption  mr-2 flex items-center">Popular:</span>
      {FILTERS.map((f) => (
        <button
          key={f.key}
          onClick={() => onToggle(f.key)}
          className={`px-4 py-1.5 rounded-full border border-black text-caption font-caption transition-colors cursor-pointer ${
            quickFilter === f.key
