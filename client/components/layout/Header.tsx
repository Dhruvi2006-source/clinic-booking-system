        ...
        <div className="max-w-container-max mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-1 text-center sm:text-left">
            ...
            <a href={`tel:${dummyPhone}`} className="flex items-center gap-1.5 text-white hover:text-secondary/80 transition-colors font-semibold">
              <span className="material-symbols-outlined text-[14px] font-fill" style={{ fontVariationSettings: "'FILL' 1" }}>call</span>
              Call Us: {dummyPhone}
            </a>
          </div>
          {/* Notice & Status */}
      ...
      <header className="w-full bg-[#0B1F3A] text-white border-b border-slate-800">
        <div className="flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto h-20">
          ...
          <nav className="hidden md:flex gap-8 items-center h-full">
            <Link
              href="/"
              className={`font-label-md text-label-md transition-all duration-200 h-full flex items-center border-b-2 pt-[2px] text-white ${
                pathname === "/"
            ...
            >
              <button
                className={`font-label-md text-label-md transition-all duration-200 h-full flex items-center gap-1 border-b-2 pt-[2px] cursor-pointer text-white ${
              ...
              {servicesDropdownOpen && (
                <div className="absolute top-[80px] left-1/2 -translate-x-1/2 w-[600px] bg-[#071527] rounded-2xl p-6 shadow-2xl border border-slate-800 animate-pop-in grid grid-cols-3 gap-6 text-left">
                  ...
                  <div className="space-y-3">
                    <h4 className="text-xs uppercase tracking-wider font-bold text-secondary flex items-center gap-1">
                      <span className="material-symbols-outlined text-[14px]">health_metrics</span>
                      Preventive Care
                    </h4>
                    <ul className="space-y-1.5 text-xs text-slate-300 font-medium">
                  ...
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-1.5 text-xs text-slate-300 font-medium">
                      <li className="text-red-400/90">Life-Threatening Emergencies</li>
                    ...
                    <div className="pt-2 border-t border-slate-800 text-[10px] text-slate-400">
                      *Please dial 911 or head to the nearest Emergency Room for critical issues.
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-4">
              <Link
                href="/portal"
                className="font-label-md text-label-md text-white/90 hover:text-white transition-colors duration-200 font-semibold"
              >
                Patient Portal
              </Link>
              <Link
          ...
          <div className="md:hidden bg-[#071527] border-b border-slate-800 p-6 space-y-4 animate-pop-in text-white">
            <nav className="flex flex-col gap-4">
              ...
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`font-label-md text-label-md ${
                  pathname.startsWith("/doctors") ? "text-secondary font-bold" : "text-white"
                }`}
              >
                Find a Doctor
              </Link>
              ...
              <div className="space-y-2">
                ...
                {mobileServicesOpen && (
                  <div className="pl-4 py-2 border-l border-slate-800 space-y-3 text-xs text-slate-300">
                    <div>
                      <h5 className="font-bold text-secondary uppercase tracking-wider mb-1">Illnesses</h5>
                      <p>Sore Throat, Sinusitis, UTI, Coughs, Skin Issues, Sprains, Lacerations</p>
                    </div>
                    <div>
                      <h5 className="font-bold text-secondary uppercase tracking-wider mb-1">Preventive</h5>
                      <p>Annual Exams, Pre-Employment, Clearances, Screenings (Cholesterol, Diabetes, BP, STD)</p>
                    </div>
                    <div>
                      <h5 className="font-bold text-red-400 uppercase tracking-wider mb-1">Not Treated</h5>
                      <p>Life-Threatening Emergencies, Chest Pain, Compound Fractures</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`font-label-md text-label-md ${
                  pathname === "/in-clinic" ? "text-secondary font-bold" : "text-white"
                }`}
              >
                Lounge Experience
              </Link>
